...
BEGIN
    ...
    FOR v_item IN SELECT * FROM public.import_ai_items WHERE job_id = p_job_id ORDER BY idx ASC LOOP
        ...
        -- Merge: item atual tem dados reais, item anterior no mesmo path é fragmento vazio
        IF v_item.composition_code IS NULL
           AND v_item.item_path IS NOT NULL
           AND COALESCE(v_item.quantity, 0) > 0
           AND COALESCE(v_item.unit_price, 0) > 0 THEN
            -- Verifica se existe fragmento anterior no mesmo path sem dados
            IF EXISTS (
                SELECT 1 FROM public.budget_items
                WHERE budget_id = v_budget_id
                  AND item_number = v_item.item_path
                  AND COALESCE(quantity, 0) = 0
                  AND COALESCE(unit_price, 0) = 0
                  AND type = 'insumo'
            ) THEN
                UPDATE public.budget_items
                SET quantity = COALESCE(v_item.quantity, 0),
                    unit_price = COALESCE(v_item.unit_price, 0),
                    total_price = COALESCE(v_item.quantity, 0) * COALESCE(v_item.unit_price, 0),
                    final_price = COALESCE(v_item.quantity, 0) * COALESCE(v_item.unit_price, 0),
                    hydration_status = 'pending_review',
                    updated_at = NOW()
                WHERE budget_id = v_budget_id
                  AND item_number = v_item.item_path
                  AND COALESCE(quantity, 0) = 0
        ...
        INSERT INTO public.budget_items (budget_id, user_id, level, parent_id, description, unit, quantity, unit_price,
            total_price, final_price, type, source, code, source_import_item_id, order_index, hydration_details, hydration_status, item_number)
        VALUES (v_budget_id, v_job.user_id, 3, v_parent_id, v_clean_description, COALESCE(v_item.unit, 'UN'),
            ...
            CASE
                ...
                ELSE 'pending_review'
            END,
            v_numbering -- mapped item_number
            )
