function parsePricesSheet(sheet: XLSX.WorkSheet, sheetName: string): {
    ...
    // Aliases para headers de aba de preÃ§os
    const keyAliases = ['codigo', 'preco', 'valor', 'custo', 'tipo'];

    const headerRow = findHeaderRow(data, keyAliases);

    if (headerRow === -1) {
        console.error(`[SINAPI PARSER PRICES] aba=${sheetName} ERRO: Header nÃ£o encontrado`);
    ...
    if (codeCol.index === -1) {
        console.error(`[SINAPI PARSER PRICES] ERRO: Coluna CÃ“DIGO nÃ£o encontrada`);
        return { inputPrices, compositionPrices };
    }

    if (priceCol.index === -1) {
        console.error(`[SINAPI PARSER PRICES] ERRO: Coluna PREÃ‡O nÃ£o encontrada`);
        console.log(`[SINAPI PARSER PRICES] Headers disponÃ­veis:`, normalizedHeaders);
        return { inputPrices, compositionPrices };
    }
    ...
    for (let i = headerRow + 1; i < data.length; i++) {
        ...
        // Determinar se Ã© insumo ou composiÃ§Ã£o
