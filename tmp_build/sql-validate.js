...
async function processItems(items, totalCol) {
    ...
    const subtotalGrupo = subtotalRows?.reduce((acc, curr) => acc + (Number(curr[totalCol]) || 0), 0) || 0;
    // 4) Total Geral
    const { data: allItems } = await supabase
        .from('budget_items')
        .select(totalCol)
        .eq('budget_id', budgetId);
    const totalGeral = allItems?.reduce((acc, curr) => acc + (Number(curr[totalCol]) || 0), 0) || 0;
    // 5) Peso calculations
