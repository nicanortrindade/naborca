...
async function runSnapshot() {
    ...
    // 2) Evolution (Last 15m)
    {
        ...
        if (error) console.error("Error Q2:", error);
            ...
            console.log("\n--- Q2: Recent Tasks (Last 15m) ---");
            console.table(data);
        }
    }

    // 3) Summaries (Last 15m)
    {
        const { data, error } = await supabase
        ...
        if (error) console.error("Error Q3:", error);
            ...
            console.log("\n--- Q3: Recent Summaries (Last 15m) ---");
            const mapped = data?.map(r => {
                const h = r.header || {};
                return {
                    created_at: r.created_at,
                    // task_id: h.task_id || "N/A", // Shorten output
                    kind: h.kind || "N/A",
    ...
    // 4) Items Created (Last 15m) - Group by Job/File ID (Proxy)
    {
        const { data, error } = await supabase
