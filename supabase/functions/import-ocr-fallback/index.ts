...
const STAGEB_BUILD_SIG = "stageb-stfix2-2026-02-21";
...
serve(async (req: Request) => {
    ...
    try {
        ...
        const { count: finalCount } = await supabase.from("import_ai_items").select("*", { count: "exact", head: true }).eq("job_id", job_id);
