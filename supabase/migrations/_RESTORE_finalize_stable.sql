...
BEGIN
    ...
    ELSE
        ...
        VALUES (v_job.user_id, 'Or├ºamento Importado ' || to_char(now(), 'DD/MM HH24:MI'), 'draft',
            v_uf, v_competence,
    ...
    LOOP
                ...
                BEGIN
                    ...
                    IF v_n1_id IS NULL THEN
                        ...
                        VALUES (v_budget_id, v_job.user_id, 1,
                            COALESCE(v_inferred_name, 'SE├ç├âO ' || v_n1_key),
                        ...
                        IF v_inferred_name IS NOT NULL AND EXISTS (
                            ...
                            SET description = v_inferred_name || ' (Se├º├úo ' || v_n1_key || ')'
                            WHERE id = v_n1_id;
                        END IF;
                    END IF;
                    ...
                    -- Title items (no code, no price) ÔåÆ create N2 group + CONTINUE
                    -- Data items (has price/qty) ÔåÆ insert directly under N1
                    -- =========================================================
                    IF v_path_depth = 2 THEN
                        IF v_item.composition_code IS NULL
                           ...
                           AND COALESCE(v_item.quantity, 0) = 0 THEN
                            ...
                            CONTINUE;
                        ELSE
                            -- depth=2 item WITH data ÔåÆ insert directly under N1
                            v_parent_id := v_n1_id;
                            v_level     := 3;
                        END IF;
            ...
            ELSE
                ...
                ELSIF v_level = 2 THEN
                    ...
                    IF v_parent_id IS NULL THEN
                        ...
                        VALUES (v_budget_id, v_job.user_id, 1, 'IMPORTA├ç├âO AUTOM├üTICA', 'group', 0)
        ...
        -- These are OCR fragments like "INSTALA├ç├âO. AF_08/2021" that got
        ...
        IF v_clean_description ILIKE '%INSTALA├ç├òES HIDROSSANIT├üRIAS%'
