-- =============================================================================
-- Migration M11: Fix group name inference - use current item description directly
-- Fixes: GRUPO 12.2 → TETO, GRUPO 16.3 → SPDA
-- Root cause: titles with NULL unit_price/quantity were not found by subquery
-- Fix: when path_depth = 2 and item is a title (composition_code IS NULL),
--      use v_clean_description of the current item directly as group name
-- =============================================================================
                        ...
                        IF v_item.composition_code IS NULL
                           ...
                           AND COALESCE(v_item.quantity, 0) = 0 THEN
                            ...
                            IF v_n2_id IS NULL THEN
                                -- M11 FIX: usa description do item atual diretamente
                                INSERT INTO public.budget_items
