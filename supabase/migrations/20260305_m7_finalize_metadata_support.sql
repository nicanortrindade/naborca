-- ============================================================================
-- NABORÇA • FINALIZE: M7 — Suporte a Nome da Obra e BDI Equipamentos
-- Migration: 20260305_m7_finalize_metadata_support.sql
-- Data: 2026-03-05
-- Objetivo: Utilizar o nome da obra (obra_nome) e município passados na revisão
--           além de aplicar bdi_equipamentos se fornecido.
-- ============================================================================
...
DECLARE -- Context
    ...
    -- M8: Filter bases
    v_sel_bases   text[];
    -- C6: Segunda passada para resolver títulos genéricos
    v_title_item record;
BEGIN -- 1. Setup & Validation
    ...
    ELSE
        ...
        VALUES (v_job.user_id, 
            COALESCE(NULLIF(v_obra_nome, ''), 'Orçamento Importado ' || to_char(now(), 'DD/MM HH24:MI')), 
    ...
    FOR v_item IN SELECT * FROM public.import_ai_items WHERE job_id = p_job_id ORDER BY idx ASC LOOP 
        ...
        IF v_clean_code = '0' AND COALESCE(v_item.unit_price, 0) = 0 AND COALESCE(v_item.quantity, 0) = 0 THEN
            CONTINUE; -- O título será resolvido na segunda passada abaixo
        END IF;
    ...
    -- ============================================================================
    -- C6: SEGUNDA PASSADA — Resolver títulos genéricos 
    -- ============================================================================
    FOR v_title_item IN
