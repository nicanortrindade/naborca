-- ============================================================================
-- NABOORÇA • FINALIZE: OPTIMIZE HYDRATION LOOP & PERFORMANCE
-- Migration: 20260225145000_optimize_finalize_logic.sql
-- Data: 2026-02-25
-- ============================================================================

-- 1. Optimize find_composition_in_bases to avoid expensive OR/EXISTS
...
-- 2. Update finalize function with hydration optimization
...
DECLARE -- Context
    ...
    v_n3_id       uuid;
BEGIN 
    -- 0. Set local timeout to handle large budgets (5 minutes)
    ...
    FOR v_item IN SELECT * FROM public.import_ai_items WHERE job_id = p_job_id ORDER BY idx ASC LOOP 
        ...
        IF v_clean_code = '0' AND COALESCE(v_item.unit_price, 0) = 0 AND COALESCE(v_item.quantity, 0) = 0 THEN
            -- Renomeia grupo genérico pelo path_key, não pelo v_parent_id
        ...
        IF v_clean_code != '0' THEN
            ...
            -- Optimized: Single call via INSERT ... SELECT. Total hydration success tracked by ROW_COUNT.
            ...
            GET DIAGNOSTICS v_hydration_count_a := ROW_COUNT;

            IF v_hydration_count_a > 0 THEN
