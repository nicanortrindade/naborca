-- ============================================================================
-- NABOORÇA • FINALIZE: USE find_composition_in_bases IN HYDRATION A
-- Migration: 20260223000002_finalize_use_multibase.sql
-- Data: 2026-02-23
-- Objetivo: Substituir find_internal_composition por find_composition_in_bases
--           no bloco Hydration A de finalize_import_to_budget, permitindo que
--           bases externas (ORSE, CPU, EMOP, etc.) também sejam consultadas.
--           Apenas o bloco Hydration A muda; B (analytic) e C (pending) ficam
--           intactos.
--
-- DIFF em relação a 20260220000000_fix_finalize_compositions_user_id.sql:
--   REMOVE:
--     IF EXISTS (SELECT 1 FROM public.find_internal_composition(...))
--     INSERT … FROM public.find_internal_composition(...)
--       jsonb_build_object('source', 'internal_db', ...)
--   ADICIONA:
--     IF EXISTS (SELECT 1 FROM public.find_composition_in_bases(..., v_job.user_id, ...))
--     INSERT … FROM public.find_composition_in_bases(..., v_job.user_id, ...)
--       jsonb_build_object('source', source_base, ...)
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- Verification query (run manually after db push):
--   SELECT obj_description('public.finalize_import_to_budget(uuid,uuid,jsonb,jsonb)'::regprocedure);
--   -- Or check the body directly:
--   SELECT prosrc FROM pg_proc WHERE proname = 'finalize_import_to_budget' LIMIT 1;
--   -- Confirm 'find_composition_in_bases' appears and 'find_internal_composition' does NOT.
-- ---------------------------------------------------------------------------
