-- ============================================================================
-- NABOORÇA • FINALIZE: C6 + M6 — Fix título genérico e M6 BDI
-- Migration: 20260305_m6_custom_bdi_on_import.sql
-- Data: 2026-03-05
-- Problema: títulos de seção (ex: item_path='12.2', desc='TETO') chegam antes
--           do primeiro filho (ex: '12.2.1'), então o UPDATE não encontra o 
--           budget_item ainda. Solução: segunda passada pós-loop que resolve 
--           todos os nós ainda genéricos usando import_ai_items como referência.
--           + M6: Gravar bdi_percent no custom_bdi do budget_items se presente
-- ============================================================================
...
BEGIN -- 1. Setup & Validation
    ...
    FOR v_item IN SELECT * FROM public.import_ai_items WHERE job_id = p_job_id ORDER BY idx ASC LOOP 
        ...
        -- NOTE: o UPDATE de título foi movido para a segunda passada pós-loop (C6)
        -- porque o nó pai pode ainda não existir neste momento da iteração.
        ...
        VALUES (v_budget_id, v_job.user_id, 3, v_parent_id, v_clean_description, COALESCE(v_item.unit, 'UN'), 
            ...
            CASE
                WHEN v_item.bdi_percent IS NOT NULL
                THEN v_item.bdi_percent::numeric
                ELSE NULL
            END)
    ...
    -- ============================================================================
    -- C6: SEGUNDA PASSADA — Resolver títulos genéricos após criação de todos os nós
    -- Agora todos os budget_items com path_key existem; podemos renomear sem problemas.
    -- ============================================================================
    FOR v_title_item IN
