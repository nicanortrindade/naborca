...
DROP FUNCTION IF EXISTS public.finalize_ocr_job(uuid, text, text, int);
...
begin -- 1. Discover the actual type of the 'status' column dynamically
...
if v_enum_type is null
or v_enum_type = 'unknown' then v_enum_type := 'public.import_job_status';
-- Default fallback
end if;
...
execute format(
  ...
  p_last_error,
  p_last_error,
  coalesce(p_retry_count::text, 'null'),
  coalesce(p_retry_count::text, 'null')
) using p_id;
end;
$$;