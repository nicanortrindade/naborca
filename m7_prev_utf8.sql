-- ============================================================================
-- NABOR├çA ÔÇó FINALIZE: M7 ÔÇö Suporte a Nome da Obra e BDI Equipamentos
-- Migration: 20260305_m7_finalize_metadata_support.sql
-- Data: 2026-03-05
-- Objetivo: Utilizar o nome da obra (obra_nome) e munic├¡pio passados na revis├úo
--           al├®m de aplicar bdi_equipamentos se fornecido.
-- ============================================================================
...
DECLARE -- Context
    ...
    -- M8: Filter bases
    v_sel_bases   text[];
    -- C6: Segunda passada para resolver t├¡tulos gen├®ricos
    v_title_item record;
BEGIN -- 1. Setup & Validation
    ...
    IF NOT EXISTS (
        ...
        WHERE job_id = p_job_id
            AND (description NOT LIKE 'Falha na leitura autom├ítica%')
    ) THEN 
    ...
    -- M7 Fields
    v_obra_nome := p_params->>'obra_nome';
    v_municipio := p_params->>'municipio';
    v_bdi_equip := COALESCE((p_params->>'bdi_equipamentos')::numeric, 0);

    -- M8 Fields
    IF p_params ? 'bases_selecionadas' THEN
        v_sel_bases := ARRAY(SELECT jsonb_array_elements_text(p_params->'bases_selecionadas'));
    END IF;
    ...
    IF v_job.result_budget_id IS NOT NULL AND EXISTS (SELECT 1 FROM public.budgets WHERE id = v_job.result_budget_id) THEN
        ...
        UPDATE public.budgets SET 
            name = COALESCE(NULLIF(v_obra_nome, ''), name),
            settings = p_params, 
            updated_at = now(), 
            sinapi_uf = v_uf, 
    ...
    ELSE
        ...
        VALUES (v_job.user_id, 
            COALESCE(NULLIF(v_obra_nome, ''), 'Or├ºamento Importado ' || to_char(now(), 'DD/MM HH24:MI')), 
            'draft', v_uf, v_competence,
    ...
    VALUES (v_budget_id, v_job.user_id, 1, 'IMPORTA├ç├âO AUTOM├üTICA', 'group', 1)
    ...
    FOR v_item IN SELECT * FROM public.import_ai_items WHERE job_id = p_job_id ORDER BY idx ASC LOOP 
        ...
        IF v_use_parser THEN
                ...
                BEGIN
                    ...
                    IF v_n1_id IS NULL THEN
                        ...
                        VALUES (v_budget_id, v_job.user_id, 1,
                            'SE├ç├âO ' || v_n1_key, 'group', v_items_processed,
            ...
            ELSE
                -- Fallback: parser por regex na descri├º├úo (itens sem item_path)
        ...
        IF v_clean_code = '0' AND COALESCE(v_item.unit_price, 0) = 0 AND COALESCE(v_item.quantity, 0) = 0 THEN
            CONTINUE; -- O t├¡tulo ser├í resolvido na segunda passada abaixo
        END IF;

        -- Skip multiline duplicate
        ...
        -- Check 3: Skip coded multiline duplicate
        ...
        INSERT INTO public.budget_items (budget_id, user_id, level, parent_id, description, unit, quantity, unit_price, 
            total_price, final_price, type, source, code, source_import_item_id, order_index, hydration_details, custom_bdi)
        ...
        IF v_clean_code != '0' THEN
            -- ÔöÇÔöÇ Hydration A (SINAPI + Bases Externas) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
            IF EXISTS (SELECT 1 FROM public.find_composition_in_bases(v_clean_code, v_job.user_id, v_uf, v_competence, v_desonerado, v_sel_bases)) THEN
                ...
                FROM public.find_composition_in_bases(v_clean_code, v_job.user_id, v_uf, v_competence, v_desonerado, v_sel_bases);
                v_found_path := 'internal_db'; v_items_hydrated_a := v_items_hydrated_a + 1;
            
            -- ÔöÇÔöÇ Hydration B (Analytic) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    -- ============================================================================
    -- C6: SEGUNDA PASSADA ÔÇö Resolver t├¡tulos gen├®ricos 
    -- ============================================================================
    FOR v_title_item IN
    ...
    LOOP
        ...
        SET description = v_title_item.description
        WHERE budget_id = v_budget_id
          AND hydration_details->>'path_key' = v_title_item.item_path
          AND (description LIKE 'SE├ç├âO %' OR description LIKE 'GRUPO %' OR description LIKE 'SUBGRUPO %');
    END LOOP;
